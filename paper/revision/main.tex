...
\begin{figure}[h!]
    \centering
    \captionsetup{justification=centering}
    \includegraphics[width=0.7\columnwidth]{Main/images/System_Workflow.png}
...
\centering
\captionsetup{justification=centering}
\includegraphics[width=\columnwidth]{Main/images/case_A_no_replan(+10).png}
...
\centering
\captionsetup{justification=centering}
\includegraphics[width=\columnwidth]{Main/images/case_B_single_replan(-40).png}
\caption{CASE B:Single replanning}
\label{fig:imgb}
\end{figure}


\added{Case B introduces a conflict between the nominal trajectory and an obstacle safety region. PORCE generates a local detour around the obstacle and rejoins the nominal path once the conflict is cleared, preserving the original mission structure. As there is only one obstacle in conflict with the nominal path, PORCE only has to compute an initial plan, and after the conflict is solved, the UAS rejoins the nominal trajectory before reaching to the nominal front-edge point.}

\begin{figure}[h!]
\centering
\captionsetup{justification=centering}
\includegraphics[width=\columnwidth]{Main/images/case_C_multiple_replans(-40).png}
\caption{CASE C: Multiple replannings}
\label{fig:imgC}
\end{figure}


\added{Case C considers a situation in which an avoidance trajectory encounters additional conflicts, as the space is covered by more detected obstacles. The planner is therefore invoked repeatedly, producing successive local updates while maintaining the same mission objective. The figure shows an example in which PORCE has to work from the beginning, as there is an obstacle already in the reaction margin horizon. After executing all the replans, the UAS reacquires the nominal trajectory before reaching the nominal-front edge point.}

\begin{figure}[h!]
\centering
\captionsetup{justification=centering}
\includegraphics[width=\columnwidth]{Main/images/case_D_failsafe(-40).png}
\caption{CASE D: Failsafe scenario}
\label{fig:imgD}
\end{figure}

\added{Case D represents a constrained configuration in which no admissible route satisfies the obstacle constraints. Rather than forcing a trajectory through the blocked region, PORCE returns a Failsafe outcome. As it is a frozen size scenario, there is no option to search for an alternative path near the left side, so after searching for other alternative paths, PORCE triggers the Failsafe.}
...
\caption{\added{Full logged mission of the archived audited run 20260731\_061815: (a) complete mission with the executed evasion segment (orange) and route reattachment onto the nominal corridor (inset: evasion window enlarged, centred on the obstacle estimate); (b) rejoin detail with the planner 4~m raster (light grid, anchored at the vehicle position at the planning call), the raster cells blocked by the 12~m exclusion disc, and the executed bypass, which follows the pruned A* route along grid edges or vertex-to-vertex diagonals at the reduced replanning speed.}}
